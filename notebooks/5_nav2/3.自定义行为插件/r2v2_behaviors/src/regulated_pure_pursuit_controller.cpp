  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  const double carrot_dist2 =
    ...
    double dtheta = atan2(carrot_pose.pose.position.y, carrot_pose.pose.position.x);
    cmd_vel.twist.linear.x = linear_vel*cos(dtheta);
    cmd_vel.twist.linear.y = linear_vel*sin(dtheta);
    //cmd_vel.twist.angular.z = rotate_to_heading_angular_vel_ * linear_vel *  tf2::getYaw(transformed_plan.poses.back().pose.orientation);
